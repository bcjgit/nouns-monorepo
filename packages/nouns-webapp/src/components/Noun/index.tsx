...
export const NounCircleImg: React.FC<{ imgPath: string; alt: string; isSmall: boolean}> = props => {
  const { imgPath, alt, isSmall}  = props;
  
  return <Image className={`${classes.img} ${isSmall ? classes.smallImg : classes.largeImg}`} src={imgPath ? imgPath : loadingNoun} alt={alt} fluid roundedCircle/>;
}; 
