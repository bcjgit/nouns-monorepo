...
const nounVoteActivityRow = (proposal: Proposal, index: number, nounVoted: boolean , nounSupported: boolean, nounId: number) => {
            ...
            <Link to={proposalURL} className={classes.recentActivityLink}>
                <div className={classes.recentActivityItem}>
                    <div className={classes.activityRowItemText}>
                        {icon} &ensp; <strong>Noun {nounId}</strong> {votingInfoText} proposition <strong>{proposal.title}</strong>
                    </div>
                </div>
            </Link>
        </li>
            ...
            <Row>
                ...
                {proposals?.length ? (
                    ...
                    .map((p: Proposal, i : number) => {
                        return nounVoteActivityRow(p, i, proposalsVotedOn.includes(p.id), supportedProposals.includes(p.id), nounId);
                    })
                    ): (
                    <div>
                        no data
